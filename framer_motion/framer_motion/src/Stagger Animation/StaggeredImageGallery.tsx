...
const StaggeredImageGallery = () => {
  return (
    <motion.div className='flex' initial='hidden' animate='visible' variants={{visible:{transition:{staggerChildren:1}}}}>
        {
            images.map((src,index)=>(
                <motion.img key={index} src={src} alt={`img${index}`} variants={imageVariants} className="w-[20rem] h-auto ml-[2rem] rounded "></motion.img>
            ))
        }
    </motion.div>
  )
